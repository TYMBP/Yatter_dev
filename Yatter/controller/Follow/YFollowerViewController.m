...
@implementation YFollowerViewController {
  NSInteger cnt;
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  [_followSharedData setFollowFlag:2];
  [self runAPI];
}
...
//  _list = [NSMutableArray array];
  ...
  LOG(@"YamadaTest:jsonStr %@", jsonStr);
  for (NSDictionary *obj in data) {
    YUserListData *followData = [[YUserListData alloc] init];
    LOG(@"YamadaTest:%s for",__func__);
